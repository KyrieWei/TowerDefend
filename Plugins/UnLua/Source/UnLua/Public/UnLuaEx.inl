...
namespace UnLua
{
    ...
    struct TArgumentIntelliSense<T1, T2...>
    {
        ...
        {
            FString TypeName = TTypeIntelliSense<typename UnLuaChooseClass<TIsPointer<T1>::Value, typename TDecay<typename TRemovePointer<T1>::Type>::Type*, typename TDecay<T1>::Type>::Result>::GetName();
    ...
    void GenerateArgsIntelliSense(FString &Buffer, FString &ArgList)
    {
        ...
        // return 
        FString ReturnTypeName = TTypeIntelliSense<typename UnLuaChooseClass<TIsPointer<RetType>::Value, typename TDecay<typename TRemovePointer<RetType>::Type>::Type*, typename TDecay<RetType>::Type>::Result>::GetName();
    ...
    */
    ...
    {
        GenerateIntelliSenseInternal(Buffer, typename UnLuaChooseClass<bIsReflected, FTrue, FFalse>::Result());
    }
    ...
    {
        AddDefaultFunctions(typename UnLuaChooseClass<bIsReflected, FTrue, FFalse>::Result());
    }
    ...
    {
        AddConstructor(typename UnLuaChooseClass<TIsConstructible<ClassType, CtorArgType...>::Value, FTrue, FFalse>::Result());
        AddDestructor(typename UnLuaChooseClass<TAnd<TIsDestructible<ClassType>, TNot<TIntegralConstant<bool, std::is_trivially_destructible_v<ClassType>>>>::Value, FFalse, FTrue>::Result());
    }

    template <bool bIsReflected, typename ClassType, typename... CtorArgType>
    void TExportedClass<bIsReflected, ClassType, CtorArgType...>::AddDefaultFunctions(FTrue Reflected)
    {
        AddDefaultFunctions_Reflected(typename UnLuaChooseClass<TPointerIsConvertibleFromTo<ClassType, UObject>::Value, FTrue, FFalse>::Result());
    }
