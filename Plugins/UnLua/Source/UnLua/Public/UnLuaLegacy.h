...
namespace UnLua
{
    ...
    */
    ...
    struct TTypeInterface : public ITypeInterface
    {
        ...
        {
            static_assert(TIsDestructible<T>::Value, "type must be destructible!");
            DestructInternal((T*)Dest, typename UnLuaChooseClass<std::is_trivially_destructible_v<T>, FTrue, FFalse>::Result());
        }
        ...
        virtual void Copy(void* Dest, const void* Src) const override
        {
            ...
            CopyInternal((T*)Dest, (const T*)Src, typename UnLuaChooseClass<TIsTriviallyCopyConstructible<T>::Value, FTrue, FFalse>::Result());
        }

        virtual bool Identical(const void* A, const void* B) const override
        {
            return IdenticalInternal((const T*)A, (const T*)B, typename UnLuaChooseClass<THasEqualityOperator<T>::Value, FTrue, FFalse>::Result());
        }
        ...
        virtual bool WriteValue(lua_State* L, void* ValuePtr, int32 IndexInStack, bool bCreateCopy) const override
        {
            ...
            CopyInternal((T*)ValuePtr, &V, typename UnLuaChooseClass<TIsTriviallyCopyConstructible<T>::Value, FTrue, FFalse>::Result());
            return false;
        }
