---
name: mvvm-ui-framework
overview: 基于UnLua实现MVVM UI管理框架，提供完整的数据绑定、UI生命周期管理和窗口堆栈功能，位于Content/Script目录下
todos:
  - id: explore-unlua-integration
    content: 使用 [subagent:code-explorer] 深入探索UnLua插件API和UMG Widget绑定机制
    status: completed
  - id: create-framework-core
    content: 创建MVVM框架核心模块（UIManager、BaseView、BaseViewModel、BaseModel）
    status: completed
    dependencies:
      - explore-unlua-integration
  - id: implement-data-binding
    content: 实现数据绑定系统和事件系统，支持双向绑定和属性通知
    status: completed
    dependencies:
      - create-framework-core
  - id: create-test-widgets
    content: 使用 [mcp:unreal-mcp] 创建测试用的UMG Widget Blueprint
    status: completed
    dependencies:
      - create-framework-core
  - id: build-examples
    content: 构建完整的示例应用，展示MVVM框架的使用方法
    status: completed
    dependencies:
      - implement-data-binding
      - create-test-widgets
  - id: framework-testing
    content: 使用 [mcp:unreal-editor] 测试框架功能和性能，验证UI生命周期管理
    status: completed
    dependencies:
      - build-examples
---
